### Animations link: https://app.lottiefiles.com/animation/d1b0b917-01af-4274-9d00-3abb1cf8cab9?panel=embed
...
function TutorDashboard() {
  ...
  const handleOpenForum = async (ctId) => {
    try {
        ...
        if (!forumData.forumId) {
            throw new Error("Forum ID not found in response");
        }
  ...
  const handleForumClick = (forumId) => {
    ...
    console.log("Navigating to forum with ID:", forumId);
    
    // Make sure forumId is defined before navigation
    if (!forumId) {
